        ...
        if buffer[0] == "Set_path":
            ...
            while Checkname==False:
                ...
                for j in range(1,len(buffer)-1):
                    if checkfristplace==False:
                        fristplace = fristplace +buffer[j]+ " "
                    else:
                        seconplace = seconplace +buffer[j]+ " "
                    if fristplace.strip(" ") in ListOfTextbuffer and fristplace.strip(' ') not in fristplacebufferlist:
                        checkfristplace = True
                #print(fristplace.strip(" "))
                #print(seconplace.strip(" "))
                if fristplace.strip(" ") in ListOfTextbuffer and seconplace.strip(" ") in ListOfTextbuffer:
                    Specialbuffer.append(fristplace.strip(' '))
                    Specialbuffer.append(seconplace.strip(' '))
                    Checkname=True
                else:
            ...
            for j in range(2,len(buffer)):
                if buffer[j] == "from":
                    ...
                    Namebuffer = ""
                elif buffer[j] in ListOfTextbuffer or buffer[j] in Nation:
...
def processOrder():
    ...
    for i in keyOrderList:
        ...
        for j in OrderDict[i].listevent:
            countevent+=1
            #if FailBool[countevent] == False: #for check truck2
